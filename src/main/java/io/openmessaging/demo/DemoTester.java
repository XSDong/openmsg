...
public class DemoTester {
    ...
    public static void main(String[] args) {
         ...
         */
        properties.put("STORE_PATH", "C:/alidata/");
        ...
        //消费样例1，实际测试时会Kill掉发送进程，另取进程进行消费
        {
            ...
            while (true) {
                ...
                if (topic != null) {
                    ...
                    ((DefaultBytesMessage)messagesForTopic1.get(topic1Offset)).debugOutput();
                    ((DefaultBytesMessage)message).debugOutput();
                    if(messagesForTopic1.get(topic1Offset).equals(message)){System.out.println("true");}
            ...
            System.out.println(String.format("Team1 cost:%d ms; producer %d ms; consumer %d ms;tps:%d q/ms", 
            		T2 + T1, T1,T2,(queue1Offset + topic1Offset)/(T1 + T2)));
